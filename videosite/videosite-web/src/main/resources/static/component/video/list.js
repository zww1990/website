import { videoListApi, videoDelApi } from '../utils/axiosapi.js'
...
export default {
  async setup() {
    ...
    const handleClick = (card, action) => {
      router.push(`/video/${action}/${card.id}`)
    }
    const handleDelete = card => {
      videoDelApi(card).then(res => {
    ...
    return { state, fallbackImg, avatarImg, handleClick, store, handleDelete }
  },
  template: `
    <a-row :gutter="[8,8]">
