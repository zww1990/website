...
export default {
  async setup() {
    ...
    return { store, fallbackImg, avatarImg, handleClick }
  },
  template: `
    <a-row>
      <a-col :span="24">
        <span :style="{ marginRight: '8px' }">搜索结果:</span>
      </a-col>
    </a-row>
    <a-divider />
    <a-row :gutter="[8,8]">
      <template v-if="store.videos.length > 0">
        <a-col :span="4" v-for="card in store.videos" :key="card.id">
          <a-card hoverable>
            <template #cover>
